...
from chat.application.tools.web.services.web_search.schemas.tavily import (
    TavilySearchRequest,
    map_tavily_response,
)
...
_TAVILY_SEARCH_ENDPOINT = "/search"
...
class TavilySearcher(WebSearcher):
    def __init__(
        ...
        self._client = client
        self._request_url = f"{base_url.rstrip('/')}{_TAVILY_SEARCH_ENDPOINT}"
    ...
    async def search(
        ...
        request = TavilySearchRequest(query=query, max_results=max_results)
