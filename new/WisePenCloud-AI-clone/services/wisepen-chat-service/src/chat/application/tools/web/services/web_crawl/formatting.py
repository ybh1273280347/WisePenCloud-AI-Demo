...
def format_crawl_result(result: CrawlResult) -> str:
    ...
    page_items = [
        item for item in result.items if item.kind == CrawlItemKind.PAGE
    ]
    document_items = [
        item for item in result.items if item.kind == CrawlItemKind.DOCUMENT
    ]
    skipped_items = [
        item
        for item in result.items
        if item.kind in {CrawlItemKind.SKIPPED, CrawlItemKind.ERROR}
    ]
    ...
    if skipped_items:
        ...
        for item in skipped_items[:20]:
            ...
            reason = item.skip_reason.value if item.skip_reason is not None else "unknown"
