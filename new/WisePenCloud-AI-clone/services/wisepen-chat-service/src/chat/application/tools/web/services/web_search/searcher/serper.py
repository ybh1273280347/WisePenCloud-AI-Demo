...
from chat.application.tools.web.services.web_search.schemas.serper import (
    SerperSearchRequest,
    map_serper_response,
)
...
class SerperSearcher(WebSearcher):
    def __init__(
            self,
        ...
        self._client = client
        self._request_url = f"{base_url.rstrip('/')}{_SERPER_SEARCH_ENDPOINT}"
    ...
    async def search(
        ...
        if not response.results:
            raise SearchProviderTransientError(
                ...
                queries=[query],
                reason="Serper empty_result returned from upstream source",
            )

        return response


class CustomSerperSearcher(SerperSearcher):
    pass