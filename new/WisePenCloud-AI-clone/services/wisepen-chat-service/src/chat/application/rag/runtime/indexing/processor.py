...
from chat.application.rag.runtime.indexing.index_builder.index_builder import RagIndexBuilder
...
class RagIndexProcessor(RagIndexMessageProcessor):
    ...
    async def process(self, message: RagIndexMessage) -> None:
        ...
        await self._resource_service.mark_indexing(
            resource_kind=message.resource_kind,
            user_id=message.user_id,
            resource_id=message.resource_id,
            index_version=message.target_index_version,
        )
        ...
        try:
            ...
            await self._resource_service.mark_index_failed(
                resource_kind=message.resource_kind,
                user_id=message.user_id,
                resource_id=message.resource_id,
                index_version=message.target_index_version,
        ...
        await self._resource_service.mark_index_success(
            resource_kind=message.resource_kind,
            user_id=message.user_id,
            resource_id=message.resource_id,
            index_version=message.target_index_version,
