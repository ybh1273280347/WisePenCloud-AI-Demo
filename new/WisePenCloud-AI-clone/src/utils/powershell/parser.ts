...
export type ParsedCommandElement = {
  ...
  /** The AST element type from PowerShell's parsers */
...
type ParsedStatement = {
  /** The AST statement type from PowerShell's parsers */
...
/**
 * A parse error from PowerShell's parsers.
 */
type ParseError = {
  message: string
  errorId: string
...
/**
 * The complete parsed result from the PowerShell AST parsers.
 */
 ...
 * in bundled builds). The script uses the native PowerShell AST parsers to
...
export function transformCommandAst(
  ...
  if (cmdElements.length > 0) {
    ...
    // integer .value that crashes stripModulePrefix() → parsers falls through
...
/**
 * Parse a PowerShell command using the native AST parsers.
...
async function parsePowerShellCommandImpl(
  ...
  if (!trimmed) {
    logForDebugging('PowerShell parsers: empty stdout from pwsh')
    return makeInvalidResult(
      command,
      'No output from PowerShell parsers',
      'EmptyOutput',
    )
  }
  ...
  try {
    ...
    return makeInvalidResult(
      command,
      'Invalid JSON from PowerShell parsers',
      'InvalidJson',
    )
  }
}
...
/**
 * Tokenizer-level dash characters that PowerShell's parsers accepts as
 ...
 * powershell.exe (contrast with `/` which is an argv-parsers quirk of
 ...
 *
 * The parsers maps CommandParameterAst → 'Parameter' regardless of which dash
