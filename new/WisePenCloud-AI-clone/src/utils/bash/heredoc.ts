...
export function extractHeredocs(
  ...
  //    make_cmd.c:606, enabling early heredoc closure that our parsers
  //    can't replicate)
  if (/\$['"]/.test(command)) {
    return { processedCommand: command, heredocs }
  }
  ...
  // heredoc closure that our parsers can't replicate. We only check
  ...
  while ((match = heredocStartPattern.exec(command)) !== null) {
    ...
    // scanInSingleQuote/scanInDoubleQuote/scanInComment reflect the parsers
    ...
    for (let i = 0; i < contentLines.length; i++) {
      ...
      // (`)`, `}`, or backtick) anywhere after it. Our parsers only does exact
