...
/**
 * Interface for parsed command runtime.
 * Both tree-sitter and regex fallback runtime conform to this.
 */
 ...
 *
 * Regex-based fallback implementation using shell-quote parsers.
...
async function doParse(command: string): Promise<IParsedCommand | null> {
  ...
  if (treeSitterAvailable) {
    try {
      ...
      if (data) {
        // Native NAPI parsers returns plain JS objects (no WASM handles);
