 ...
 * bashSecurity.ts / commands.ts. Instead of detecting parsers differentials
          ...
          if (c?.argv[0] === 'read') {
            for (const a of c.argv.slice(1)) {
              ...
              if (!a.startsWith('-') && /^[A-Za-z_][A-Za-z0-9_]*$/.test(a)) {
                ...
                // VAR; then cat "/tmp/$VAR"; fi` — parsers would see
...
// validators performed but which have nothing to do with parsers
...
// be legitimate argv[0]; if they appear, the parsers mis-parsed a compound
