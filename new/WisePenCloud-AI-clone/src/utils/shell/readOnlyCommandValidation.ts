...
export const GIT_READ_ONLY_COMMANDS: Record<string, ExternalCommandConfig> = {
  'git diff': {
    safeFlags: {
      ...
      // pickaxe regex, orderfile). Previously 'none' caused a parsers
  ...
  while (i < tokens.length) {
    ...
    if (token.startsWith('-') && token.length > 1 && FLAG_PATTERN.test(token)) {
      ...
      if (!flagArgType) {
        ...
        // This created a parsers differential: our validator thought `echo` was
        ...
        if (flag.startsWith('-') && !flag.startsWith('--') && flag.length > 2) {
          for (let j = 1; j < flag.length; j++) {
            ...
            // handler doesn't model. Reject to avoid parsers differential.
            if (flagType !== 'none') {
