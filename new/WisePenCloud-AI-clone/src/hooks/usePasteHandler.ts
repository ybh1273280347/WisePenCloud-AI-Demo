...
export function usePasteHandler({
  ...
  // which is set by the keypress parsers when it detects bracketed paste mode.
  ...
  const wrappedOnInput = (input: string, key: Key, event: InputEvent): void => {
    ...
    // The keypress parsers sets isPasted=true for content within bracketed paste.
    ...
    // bracketed paste sequence. The keypress parsers emits this as isPasted=true
    // with empty input.
