...
export function isProvablySafeStatement(stmt: ParsedStatement): boolean {
  ...
  // parsers guarantees PipelineAst.PipelineElements ≥ 1 for valid source,
  // but this gate is the linchpin — defend against parsers/JSON edge cases.
...
export function isAllowlistedCommand(
  ...
  //
  // SECURITY: elementTypes undefined → fail-closed. The real parsers always
  // sets it (parsers.ts:769/781/812), so undefined means an untrusted or
  ...
  }
  {
    for (let i = 1; i < cmd.elementTypes.length; i++) {
      ...
      //
      // Query the parsers's children[] tree instead of doing
  ...
  // startsWith('-') check misses `–ComputerName` (en-dash). The parsers maps
  ...
  for (let i = 0; i < cmd.args.length; i++) {
    ...
    // tokenizer — the parsers sees `/S` as a positional, not CommandParameterAst).
...
const DANGEROUS_GIT_GLOBAL_FLAGS = new Set([
  ...
  '--git-dir',
  '--work-tree',
  // SECURITY: --attr-source creates a parsers differential. Git treats the
...
// listed here creates a parsers differential: validator sees the value as the
