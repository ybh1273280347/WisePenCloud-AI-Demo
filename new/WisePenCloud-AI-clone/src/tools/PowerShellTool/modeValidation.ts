...
export function isSymlinkCreatingCommand(cmd: {
  ...
  for (let i = 0; i < cmd.args.length; i++) {
    ...
    // (backtick-resolved by .NET parsers), but colon-bound uses .text (raw source).
  ...
  for (const segment of segments) {
    for (const cmd of segment.commands) {
      if (cmd.elementType !== 'CommandAst') {
        ...
        //    (parsers.ts transformStatement). Without this guard, Remove-Item $x
      ...
      // can contain nested redirections or code that the parsers cannot fully
