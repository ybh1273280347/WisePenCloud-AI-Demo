 ...
 *
 * Extracts file paths from PowerShell commands using the AST parsers
...
const CMDLET_PATH_CONFIG: Record<string, CmdletPathConfig> = {
  ...
  'set-item': {
    ...
    // so the practical write surface is registry/env/function/alias llm_clients.
...
function validatePath(
  ...
  if (normalizedPath.includes('`')) {
    ...
    // (parsers uses .value); this guard primarily fires for redirection
...
function extractPathsFromCommand(cmd: ParsedCommandElement): {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    // parsers maps CommandParameterAst → 'Parameter' regardless of dash char.
