 ...
 * Uses the parsers to get the first command name from the AST.
 */
...
export async function powershellToolHasPermission(
  ...
  // even when the parsers fails (e.g., pwsh unavailable).
  if (!parsed.valid) {
    ...
    for (const fragment of backtickStripped.split(/[;|\n\r{}()&]+/)) {
      ...
      //                                  → deny(iex:*) misses
      // The parse-succeeded path handles these via AST (parsers.ts:839 strips
      ...
      // regardless of parsers availability. Conservative: only positional
  ...
  // backtick escapes resolved by the parsers. Labeled loop breaks on FIRST
  ...
  for (const { text: subCmd, element } of allSubCommands) {
    // element.name is quote-stripped at the parsers (transformCommandAst) so
  ...
  if (subCommandsNeedingApproval.length === 0) {
    ...
    // would auto-allow here. hasAssignments is top-level-only (parsers.ts:1385)
