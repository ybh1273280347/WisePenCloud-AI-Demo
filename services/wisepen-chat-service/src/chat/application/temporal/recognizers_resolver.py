...
def resolve_time_text(
    *,
    ...
    _validate_default_recent_days(default_recent_days)
    _validate_domain_sensitivity(domain_sensitivity)

    tz = _load_timezone(timezone_name)
    timezone_name = timezone_name.strip()
    now_local = datetime.now(tz)

    mentions = _extract_mentions(input_text)
    if not mentions:
...
def _validate_default_recent_days(default_recent_days: int) -> None:
    if isinstance(default_recent_days, bool) or not isinstance(default_recent_days, int):
        raise TimeResolveError("default_recent_days must be an integer.")
    if default_recent_days < 1 or default_recent_days > 365:
        raise TimeResolveError("default_recent_days must be between 1 and 365.")


def _validate_domain_sensitivity(domain_sensitivity: Optional[str]) -> None:
    if domain_sensitivity not in (None, "high", "medium", "low"):
        raise TimeResolveError(
            "domain_sensitivity must be one of high, medium, low when provided."
        )
...
def _extract_mentions(text: str) -> List[TemporalMention]:
    mentions: List[TemporalMention] = []

    for culture in (Culture.Chinese, Culture.English):
