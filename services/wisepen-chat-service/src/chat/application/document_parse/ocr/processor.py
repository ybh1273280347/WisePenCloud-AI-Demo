...
class OcrProcessor:
    ...
    def __init__(
        ...
        if worker_mode != _WORKER_MODE_LAZY_PERSISTENT:
            raise ValueError(f"Unsupported OCR worker mode: {worker_mode}")
    ...
    async def recognize_image(self, image_path: Path) -> OcrResult:
        ...
        try:
            ...
            return OcrResult(ok=False, error="OCR processing timed out.", backend=self._backend)
    ...
    async def _recognize_image(self, image_path: Path) -> OcrResult:
        ...
        if not text:
            return OcrResult(ok=False, error="OCR produced no text.", backend=self._backend)
    ...
    async def _start_worker(self) -> asyncio.subprocess.Process:
        ...
        service_root = current.parents[5]
        env = os.environ.copy()
    ...
    async def _drain_worker_stderr(self, process: asyncio.subprocess.Process) -> None:
        ...
        while True:
            ...
            if any(keyword in lower for keyword in error_keywords):
                log_fail("OCR worker stderr", decoded)
            else:
                log_ok("OCR worker stderr", stderr=decoded)
