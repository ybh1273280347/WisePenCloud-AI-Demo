    ...
    fused = weighted_rrf(
        [
            ...
            RankedList(
                ...
                ids=sorted(ids, key=lambda item_id: (-text_overlap_scores[item_id], ids.index(item_id))),
                weight=2.0,
            ),
            RankedList(
                name="maintenance",
...
def rank_issues(
    ...
    fused = weighted_rrf(
        [
            ...
            RankedList(
                name="recency",
                ids=_rank_ids_by_score(ids, issues, lambda issue: _date_recency_score(issue.updated_at)),
                weight=0.9,
            ),
            RankedList(
                name="type_signal",
    ...
    ordered_ids = sorted(
        ...
        key=lambda item_id: (
            -text_overlap_scores[item_id],
            -_date_recency_score(by_id[item_id].updated_at),
...
def _repository_maintenance_score(repo: GitHubRepositoryResult) -> float:
    score = _date_recency_score(repo.pushed_at or repo.updated_at)
    if repo.updated_at and repo.pushed_at and repo.updated_at != repo.pushed_at:
        score += 0.15 * _date_recency_score(repo.updated_at)
    if repo.description:
        score += 0.2
...
def _is_stale_repository(repo: GitHubRepositoryResult) -> bool:
    parsed = _parse_github_datetime(repo.pushed_at or repo.updated_at)
    if parsed is None:
        return True
    age_days = (datetime.now(timezone.utc) - parsed).total_seconds() / 86400.0
    return age_days > (365.0 * 3)
...
def _date_recency_score(value: Optional[str]) -> float:
