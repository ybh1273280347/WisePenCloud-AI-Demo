...
async def collect_ax_snapshot_elements(
    ...
    *,
    ...
    if frame_depth >= SNAPSHOT_MAX_FRAME_DEPTH:
    ...
    for element in elements:
        ...
        if element.get("role") != "iframe":
            continue
        if child_frame_index >= len(frame.child_frames):
            continue
        ...
        child_path = frame_path_for_child(frame_path, child_frame_index)
        child_frame_index += 1
        try:
