...
from chat.application.tools.web.services.web_search.schemas.exa import (
    ExaSearchRequest,
    map_exa_response,
)
...
class ExaSearcher(WebSearcher):
    def __init__(
        ...
        self._client = client
        self._request_url = f"{base_url.rstrip('/')}{_EXA_SEARCH_ENDPOINT}"
    ...
    async def search(
        ...
        request = ExaSearchRequest(query=query, num_results=max_results)
        ...
        if not response.results:
            raise SearchProviderTransientError(
                ...
                queries=[query],
                reason="Exa empty_result returned from upstream source",
            )
        return response